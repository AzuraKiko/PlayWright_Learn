...
class BasePage {
    ...
    async isElementNotVisible(selector) {
        try {
            ...
            logger.error(
                `Lỗi khi kiểm tra element không hiển thị: ${selector}`,
                error
            );
            return true;
        }
    ...
    /**
     * Đếm số lượng phần tử phù hợp với selector
     * @param {string} selector - CSS hoặc XPath selector của phần tử
     * @returns {Promise<number>} - Số lượng phần tử tìm thấythấy
     */
    async getElementCount(selector) {
        try {
            ...
            logger.error(`Lỗi khi đếm số lượng phần tử: ${selector}`, error);
            return 0;
        }
    }
    ...
    async selectOption(selector, value) {
        try {
            ...
            const isNativeDropdown = await locator.evaluate(
            ...
            logger.error(
                `Lỗi khi chọn option "${value}" trong dropdown: ${selector}`,
                error
            );
            throw error;
        }
