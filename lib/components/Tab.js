...
class TabHelper extends BasePage {
    constructor(page) {
        super(page);
    }
    ...
    async closeTab(tabText) {
        try {
            await this.page.evaluate((text) => {
                ...
                const targetTab = tabs.find(tab =>
                    tab.textContent.trim().includes(text.trim()));
                // Lấy phần tử đóng tab trong tab đó
                const closeTab = targetTab?.querySelector('.lm_close_tab');

                if (closeTab) closeTab.click();
            }, tabText);

        } catch (error) {
            logger.error(`Error closing tab "${tabText}":`, error);
        }
    }
